...
<script>
	...
	import { getServersInfos, getListCabinetsByTaskId, getListServers } from 'api/results';
				...
				this.$refs.ruleForm.validate((valid) => {
					if(valid) {
						...
						if(this.filters.startTime) {
							let startTime = parseTime(this.filters.startTime, '{y}-{m}-{d} {h}:{i}:{s}');
							para.starttime = startTime;
						}
						if(this.filters.endTime) {
							let end = parseTime(this.filters.endTime, '{y}-{m}-{d} {h}:{i}:{s}');
							let times = end.split(' ')[1].split(':');
							let endTime = (times.every(i => {
								return i === "00"
							})) ? `${end.split(' ')[0]} 23:59:59` : end;
						...
						getServersInfos(self, para).then((res) => {
							if(res.data.data && res.data.data.list.length > 0) {
								...
								this.rows = res.data.data.list.map(i => {
									...
									let timeStr = parseTime(i.startTime, '{y}-{m}-{d} {h}:{i}:{s}') + '至';
									timeStr += i.endTime ? parseTime(i.endTime, '{y}-{m}-{d} {h}:{i}:{s}') : "--巡检中...";
									i.timeField = timeStr;
									return i;
								});
